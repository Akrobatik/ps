// Title : 마지막 팩토리얼 수2
// Link  : https://www.acmicpc.net/problem/2554 
// Time  : 4 ms
// Memory: 2164 KB
...
constexpr int kFTable[5] = {1, 1, 2, 6, 4};
constexpr int kPTable[4] = {6, 2, 4, 8};

int Calc(BigInt n) {
  if (n.IsZero()) return 1;

  BigInt q = n / 5;
  int rf = kFTable[n.digits[0] % 5];
  if (q.IsZero()) return rf;

  int qp = kPTable[(q.digits.size() == 1 ? q.digits[0] : q.digits[1] * 10 + q.digits[0]) % 4];
  return Calc(q) * rf * qp % 10;
}
