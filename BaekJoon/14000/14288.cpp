// Title : 회사 문화 4
// Link  : https://www.acmicpc.net/problem/14288 
// Time  : 84 ms
// Memory: 6540 KB
...
struct LCT {
  struct Node {
    Node() : l(nullptr), r(nullptr), p(nullptr), rev(0), pval(0), padd(0), sval(0), sadd(0) {}

    Node *l, *r, *p;
    int rev;
    int pval, padd, sval, sadd;
  };
  ...
  void UpdatePath(int x, int w) {
    MakeRoot(&nodes[1]);
    Access(&nodes[x]);
    Apply(&nodes[x], w);
  }

  void UpdateSub(int x, int w) {
    MakeRoot(&nodes[1]);
    Access(&nodes[x]);
    nodes[x].sadd += w;
    Pull(&nodes[x]);
  ...
  int Query(int x) {
    MakeRoot(&nodes[1]);
    Access(&nodes[x]);
    return nodes[x].pval + nodes[x].sval;
  }
 ...
 private:
  ...
  void Pull(Node* x) {
    x->sval = x->sadd;
    if (x->l) x->sval += x->l->sval;
    if (x->r) x->sval += x->r->sval;
  }
  ...
  void Push(Node* x) {
    ...
    if (x->padd) {
      if (x->l) Apply(x->l, x->padd);
      if (x->r) Apply(x->r, x->padd);
      x->padd = 0;
    }
  }
