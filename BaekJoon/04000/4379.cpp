// Title : 나누기
// Link  : https://www.acmicpc.net/problem/4379 
// Time  : 0 ms
// Memory: 2480 KB
...
int main() {
  ...
  while (cin >> t >> a >> b) {
    ...
    if (log10((double)t) * (double)(a - b) > 100.0) {
      cout << "is not an integer with less than 100 digits.\n";
      continue;
    }

    BigInt x = 0, m = 1, p = Power(t, b);
    for (int i = 0; i < a; i += b) {
      x += m;
      m *= p;
    }
