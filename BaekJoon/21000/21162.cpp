// Title : 뒤집기 K
// Link  : https://www.acmicpc.net/problem/21162
// Time  : 80 ms
// Memory: 10024 KB
...
struct LCP {
 ...
 private:
  ...
  void Kasai(const vector<int>& sv) {
    int n = sv.size() - 1;
    vector<int> ranks(n);
    for (int i = 0; i < n; i++) ranks[sa[i]] = i + 1;
    for (int i = 0, k = 0; i < n; i++, k && --k) {
...
int main() {
  ...
  copy(arr.begin(), arr.begin() + n, arr.begin() + n);
  ...
  while (k--) {
    ++idx;
    while (sa[idx] >= n || sa[idx] == 0) ++idx;
  }
  rotate(arr.begin(), arr.begin() + sa[idx], arr.begin() + n);
