// Title : 판게아 2
// Link  : https://www.acmicpc.net/problem/10724 
// Time  : 3848 ms
// Memory: 16208 KB
...
struct LCT {
  struct Node {
    Node() : l(nullptr), r(nullptr), p(nullptr), maxx(this), val(0), rev(0) {}
  ...
  pair<Node*, int> GetMaxEdge(int u, int v) {
    MakeRoot(u);
    Access(&nodes[v]);
    return {nodes[v].maxx, nodes[v].maxx - nodes.data()};
  }
 ...
 private:
  ...
  void Pull(Node* x) {
    x->maxx = x;
    if (x->l && x->maxx->val < x->l->maxx->val) x->maxx = x->l->maxx;
    if (x->r && x->maxx->val < x->r->maxx->val) x->maxx = x->r->maxx;
  }
...
int main() {
  ...
  while (t--) {
    ...
    cin >> n >> m;
    LCT lct((n << 1) + m);
    vector<pair<int, int>> memo((n << 1) + m);
    int64_t sum = 0;
    for (int i = 1; i < n; i++) {
      int u, c, x = i + n;
      cin >> u >> c;
    ...
    while (m--) {
      ...
      auto [maxx, id] = lct.GetMaxEdge(u, v);
      if (maxx->val > c) {
        auto [a, b] = memo[id];
        sum += (c - maxx->val);
        lct.Cut(id, a);
