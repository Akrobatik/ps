// Title : 책 구매하기 3
// Link  : https://www.acmicpc.net/problem/11407 
// Time  : 260 ms
// Memory: 2568 KB
...
int main() {
  ...
  vector<vector<int>> crr(m, vector<int>(n));
  for (auto& vec : crr) {
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      int x;
      cin >> x;
      mcmf.AddEdge(j, n + i, crr[i][j], x);
    }
    mcmf.AddEdge(n + i, dst, brr[i], 0);
  }

  for (int i = 0; i < n; i++) {
    mcmf.AddEdge(src, i, arr[i], 0);
  }

  auto [flow, cost] = mcmf.Solve(src, dst);
  cout << flow << "\n"
       << cost;
