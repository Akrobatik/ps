// Title : 트리의 외심
// Link  : https://www.acmicpc.net/problem/17399 
// Time  : 484 ms
// Memory: 6828 KB
...
struct LCT {
  struct Node {
    Node() : l(nullptr), r(nullptr), p(nullptr), sz(1), sum(1), rev(0) {}
...
int main() {
  ...
  auto Find = [&](int a, int b, int c) {
    array<tuple<int, int, int>, 3> arr = {{
        {lct.GetDistance(a, b), a, b},
        {lct.GetDistance(b, c), b, c},
        {lct.GetDistance(c, a), c, a},
    }};
    ...
    int w = lct.GetNth(x, y, (d >> 1) + 1);
    if (w == 0) return -1;

    d = lct.GetDistance(w, a);
    if (d == lct.GetDistance(w, b) && d == lct.GetDistance(w, c)) return w;
    return -1;
  };
