...
// Link  : https://www.acmicpc.net/problem/16415
// Time  : 12 ms
// Memory: 3460 KB
...
struct LCP {
 ...
 public:
  pair<int, int> Init(const string_view sv) {
    int n = sv.size();
    buf.resize(n + 1);
 ...
 private:
  pair<int, int> Kasai(const string_view sv) {
    int n = sv.size(), maxx = 0, pos;
    vector<int> ranks(n);
    ...
    for (int i = 0, k = 0; i < n; i++, k && --k) {
      ...
      lcp[ranks[i]] = k;
      if (maxx < k || (maxx == k && ranks[pos] > ranks[i])) maxx = k, pos = i;
    }
    return {pos, maxx};
  }
...
int main() {
  ...
  LCP solver;
  auto [pos, len] = solver.Init(s);
  cout << string_view(s.data() + pos, len);
