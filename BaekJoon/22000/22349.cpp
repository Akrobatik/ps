...
// Link  : https://www.acmicpc.net/problem/22349
// Time  : 516 ms
// Memory: 54420 KB
...
int lens[kMax], ids[kMax];
vector<int> memo[(kMax << 1) + 1];
...
int main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i < n; i++) {
      if (s1[i] == ')') memo[ids[i]].push_back(i + 1);
    }

    int ans = 0;
    for (int i = 0; i < n; i++) {
      if (s1[i] == ')' || lens[i] <= ans) continue;

      int e = i + lens[i];
      auto& outer = memo[ids[i] - 1];
      if (auto it = lower_bound(outer.begin(), outer.end(), i); it != outer.end()) {
        e = min<int>(e, *it);
      }

      auto& m = memo[ids[i]];
      auto it = upper_bound(m.begin(), m.end(), e);
      if (it-- != m.begin() && *it > i) {
        ans = max<int>(ans, *it - i);
      }
    }
