// Title : 최장 공통 부분 문자열
// Link  : https://www.acmicpc.net/problem/9249
// Time  : 16 ms
// Memory: 5092 KB
...
int main() {
  ...
  int n1 = s1.size(), n = lcp.size(), ans = 0, idx;
  for (int i = 1; i < n; i++) {
    if ((sa[i - 1] < n1) + (sa[i] < n1) != 1) continue;
    if (ans < lcp[i]) ans = lcp[i], idx = sa[i];
  }
  cout << ans << "\n"
       << string_view(s.data() + idx, ans);
