...
bool FindPath(int n, int m, int k, int sy, int sx, int ty, int tx, string& out) {
  ...
  auto Shrink = [&](int limit) {
    ...
    // cerr << m << " " << n << " " << sx << " " << sy << " " << tx << " " << ty << " " << k << "\n";
    // for (auto e : rpath) cerr << e.type << " " << e.len << "\n";
    // cerr << "\n";
    if (mit == rpath.begin()) exit(0);
...
int main() {
  ...
  while (t--) {
    ...
    if (FindPath(n, m, k, sy, sx, ty, tx, path)) {
      cout << path << "\n";
    } else {
      cout << "-1\n";
    }
  ...
  // cin >> t;
  // while (t--) {
  //   int n = gen() % 30 + 1, m = gen() % 29 + 2;
  //   int sy = gen() % n + 1, sx = gen() % m + 1;
  //   int ty = gen() % n + 1, tx = gen() % m + 1;
  //   while (sy == ty && sx == tx) ty = gen() % n + 1, tx = gen() % m + 1;
  //   int ub = CalcUpperBound(n, m, sy, sx, ty, tx);
  //   int lb = abs(sy - ty) + abs(sx - tx) + 1;
  //   int k = (gen() % 2 ? ub : lb);

  //   FindPath(n, m, k, sy, sx, ty, tx, path);
  //   if (k == path.size() + 1) continue;
